...
ggplot(data = coverage_position_count[[3]], aes(x = position, y = count, fill = factor(Tool, levels = c("MaxQuant", "MSFragger", "Casanovo")))) +
  ...
  scale_x_continuous(limits = c(0, max(coverage_position_count[[3]]$position)), 
                     breaks = seq(0, max(coverage_position_count[[3]]$position), by = 20)) +
  scale_y_continuous(limits = c(0, 40)) +
  theme_classic() +
  #theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.position = "none") +
  theme(legend.position = "none") +
  labs(x = "VDJ region position", y = "Count")
```
...
for (i in 1:length(coverage_position_count)){
  ...
  geom_col() +
  scale_fill_manual(values = Tool_cols) +
  scale_x_continuous(limits = c(0, max(coverage_position_count[[1]]$position)), 
                     breaks = seq(0, max(coverage_position_count[[1]]$position), by = 20),
                     expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 40)) +
  theme_classic() +
  theme(#axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
        axis.text.x = element_text(vjust = 2), axis.title.x = element_text(vjust = 2.5),
