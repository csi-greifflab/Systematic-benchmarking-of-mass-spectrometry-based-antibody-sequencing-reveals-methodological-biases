...
ggplot(data = coverage_position_count[[1]], aes(x = position, y = count)) +
...
for (i in 1:length(coverage_position_count)){
  p_coverage_position_count[[i]] <- ggplot(data = coverage_position_count[[i]], aes(x = position, y = count)) +
  geom_rect(aes(xmin = cdr3$cdr3_start[i], xmax = cdr3$cdr3_end[i],
                ymin = -Inf, ymax = Inf), fill = "#FDFD96", alpha = 0.05) + #cdr3
  geom_col() +
  theme_classic() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), text = element_text(size = 24)) +
  labs(x = "")
