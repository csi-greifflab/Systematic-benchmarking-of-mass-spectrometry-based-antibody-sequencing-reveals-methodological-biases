...
# Use after_stat(prop) to converst stat_count into proportions and times 100 to become percent
p_peptide_length <- ggplot(All_peptides) +
  geom_bar(aes(x = Sequence_length, y = after_stat(prop*100)), fill = "#264653") +
  facet_grid(factor(enzyme, levels = c("Ct+Tryp", "Tryp", "Ct", "AspN")) ~ factor(Tool, levels = c("MaxQuant", "MSFragger", "Casanovo"))) +
