...
```{r}
n_unique_peptide_vdj <- df %>% group_by(run) %>% summarize(n_peptide = length(unique(Sequence)))
n_unique_peptide_cdr3 <-df %>% filter(is_cdr3_related == TRUE) %>% group_by(run) %>% summarize(n_peptide = length(unique(Sequence)))
```

```{r fig.height=4, fig.width=4}
ggplot(data = n_unique_peptide_vdj, aes(x = run, y = n_peptide, fill = run)) +
  geom_col(width = 0.8) +
...
```{r fig.height=4, fig.width=4}
ggplot(data = n_unique_peptide_cdr3, aes(x = run, y = n_peptide, fill = run)) +
  geom_col(width = 0.8) +
  geom_text(aes(label = n_peptide), vjust = -0.2) +
  scale_fill_manual(values = c("#48cae4", "#0096c7", "#0077B6", "#03045E")) +
  scale_x_discrete(labels = c("1", "2", "3", "4")) +
