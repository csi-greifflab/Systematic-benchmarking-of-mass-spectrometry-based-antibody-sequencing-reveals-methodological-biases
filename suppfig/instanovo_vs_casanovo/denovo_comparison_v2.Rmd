...
insta_res_v2 <- read_csv("./instanovo/antibodies_IN_preds.csv")

# Remove empty predictions
insta_res <- insta_res %>% select(experiment_name, preds, log_probs) %>% na.omit()
insta_res_v2 <- insta_res_v2 %>% select(experiment_name, predictions, log_probabilities) %>% na.omit()

#Remove modifications (e.g. (ox))
insta_res$preds <- str_replace_all(insta_res$preds, "\\s*\\([^\\)]+\\)", "")
insta_res_v2$predictions <- str_replace_all(insta_res_v2$predictions, "\\[.*?\\]", "")

# Remove all peptides less than 7 aa long
#insta_res <- insta_res %>% filter(nchar(preds) >=7)
#insta_res_v2 <- insta_res_v2 %>% filter(nchar(predictions) >=7)

# Calculate confidence score
insta_res$log_probs <-  exp(insta_res$log_probs)
insta_res_v2$log_probabilities <-  exp(insta_res_v2$log_probabilities)

denovo_df <- bind_rows(tibble(Sample = insta_res$experiment_name,
                       Peptide = insta_res$preds,
                       Score = insta_res$log_probs,
                       Tool = "Instanovo"),
                       tibble(Sample = insta_res_v2$experiment_name,
                       Peptide = insta_res_v2$predictions,
                       Score = insta_res_v2$log_probabilities,
                       Tool = "Instanovo_v1.1"))
```
...
coverage_df_hc <- tibble(Sequence_name = c("h9C12-Q97A_HC", "h9C12-WT_HC", "Brimab_HC", "Umab_HC", "PGT121_HC",
                      ...
                      Coverage_Vregion_Casanovo = NA, Coverage_Vregion_Instanovo = NA, Coverage_Vregion_Instanovo_v1.1 = NA, 
                      Coverage_CDR3_Casanovo = NA, Coverage_CDR3_Instanovo = NA, Coverage_CDR3_Instanovo_v1.1 = NA)
...
for (i in 1:nrow(coverage_df_hc)){
 coverage_df_hc$Coverage_Vregion_Casanovo[i] <- get_coverage_percent(denovo_align_filtered_HC %>% filter(Tool == "Casanovo" & Sample_number == coverage_df_hc$Sample_number[i] & Sequence_name == coverage_df_hc$Sequence_name[i]) %>% pull(Peptide), coverage_df_hc$Sequence_name[i], cdr3)
  coverage_df_hc$Coverage_Vregion_Instanovo[i] <- get_coverage_percent(denovo_align_filtered_HC %>% filter(Tool == "Instanovo" & Sample_number == coverage_df_hc$Sample_number[i] & Sequence_name == coverage_df_hc$Sequence_name[i]) %>% pull(Peptide), coverage_df_hc$Sequence_name[i], cdr3)
    coverage_df_hc$Coverage_Vregion_Instanovo_v1.1[i] <- get_coverage_percent(denovo_align_filtered_HC %>% filter(Tool == "Instanovo_v1.1" & Sample_number == coverage_df_hc$Sample_number[i] & Sequence_name == coverage_df_hc$Sequence_name[i]) %>% pull(Peptide), coverage_df_hc$Sequence_name[i], cdr3)
  ...
  coverage_df_hc$Coverage_CDR3_Instanovo[i] <- get_cdr3_coverage_percent(denovo_align_filtered_HC %>% filter(Tool == "Instanovo" & Sample_number == coverage_df_hc$Sample_number[i] & Sequence_name == coverage_df_hc$Sequence_name[i]) %>% pull(Peptide), coverage_df_hc$Sequence_name[i], cdr3)
    coverage_df_hc$Coverage_CDR3_Instanovo_v1.1[i] <- get_cdr3_coverage_percent(denovo_align_filtered_HC %>% filter(Tool == "Instanovo_v1.1" & Sample_number == coverage_df_hc$Sample_number[i] & Sequence_name == coverage_df_hc$Sequence_name[i]) %>% pull(Peptide), coverage_df_hc$Sequence_name[i], cdr3)
}
```

```{r}
tab_hc_vdj <- kable(coverage_df_hc %>% select(Sequence_name, Sample_number, Coverage_Vregion_Casanovo, Coverage_Vregion_Instanovo, Coverage_Vregion_Instanovo_v1.1), digits = 2, booktabs = T, col.names = c("mAb name", "Sample", "Vregion_Casanovo", "Vregion_Instanovo", "Vregion_Instanovo_v1.1")) %>% kableExtra::kable_styling(latex_options = c("HOLD_position","scale_down", "striped"))

tab_hc_cdr3 <- kable(coverage_df_hc %>% select(Sequence_name, Sample_number, Coverage_CDR3_Casanovo, Coverage_CDR3_Instanovo, Coverage_CDR3_Instanovo_v1.1), digits = 2, booktabs = T, col.names = c("mAb name", "Sample", "CDR3_Casanovo", "CDR3_Instanovo", "CDR3_Instanovo_v1.1")) %>% kableExtra::kable_styling(latex_options = c("HOLD_position","scale_down", "striped"))
...
for (i in 1:nrow(coverage_df_lc)){
 ...
 coverage_df_lc$Coverage_Vregion_Instanovo[i] <- get_coverage_percent(denovo_align_filtered_LC %>% filter(Tool == "Instanovo" & Sample_number == coverage_df_lc$Sample_number[i] & Sequence_name == coverage_df_lc$Sequence_name[i]) %>% pull(Peptide), coverage_df_lc$Sequence_name[i], cdr3)
  coverage_df_lc$Coverage_Vregion_Instanovo_v1.1[i] <- get_coverage_percent(denovo_align_filtered_LC %>% filter(Tool == "Instanovo_v1.1" & Sample_number == coverage_df_lc$Sample_number[i] & Sequence_name == coverage_df_lc$Sequence_name[i]) %>% pull(Peptide), coverage_df_lc$Sequence_name[i], cdr3)
 coverage_df_lc$Coverage_CDR3_Casanovo[i] <- get_cdr3_coverage_percent(denovo_align_filtered_LC %>% filter(Tool == "Instanovo" & Sample_number == coverage_df_lc$Sample_number[i] & Sequence_name == coverage_df_lc$Sequence_name[i]) %>% pull(Peptide), coverage_df_lc$Sequence_name[i], cdr3)
  ...
  coverage_df_lc$Coverage_CDR3_Instanovo_v1.1[i] <- get_cdr3_coverage_percent(denovo_align_filtered_LC %>% filter(Tool == "Instanovo_v1.1" & Sample_number == coverage_df_lc$Sample_number[i] & Sequence_name == coverage_df_lc$Sequence_name[i]) %>% pull(Peptide), coverage_df_lc$Sequence_name[i], cdr3)
}
```

```{r}
tab_lc_vdj <- kable(coverage_df_lc %>% select(Sequence_name, Sample_number, Coverage_Vregion_Casanovo, Coverage_Vregion_Instanovo, Coverage_Vregion_Instanovo_v1.1), digits = 2, booktabs = T, col.names = c("mAb name", "Sample", "Vregion_Casanovo", "Vregion_Instanovo", "Vregion_Instanovo_v1.1")) %>% kableExtra::kable_styling(latex_options = c("HOLD_position","scale_down", "striped"))

tab_lc_cdr3 <- kable(coverage_df_lc %>% select(Sequence_name, Sample_number, Coverage_CDR3_Casanovo, Coverage_CDR3_Instanovo, Coverage_CDR3_Instanovo_v1.1), digits = 2, booktabs = T, col.names = c("mAb name", "Sample", "CDR3_Casanovo", "CDR3_Instanovo", "CDR3_Instanovo_v1.1")) %>% kableExtra::kable_styling(latex_options = c("HOLD_position","scale_down", "striped"))

tab_lc_vdj
tab_lc_cdr3
```

Based on coverage percentage, Instanovo v1.1 resulted in improved sequence coverage compared to the previous version. The previous version of Instanovo performed slightly worse than Casanovo, but Instanovo v1.1 performed slightly better.
```{r}
...
make_coverage_plot <- function(ab_name, df) {
  ...
  p_coverage <- ggplot(df_cdr3_tmp, aes(x=value, y=y)) +
    ...
    scale_colour_manual(values = c("#aa50d0", "#030304", "#669bbc")) +
...
# Heavy chain V region coverage by denovo tool: Casanovo (violet), Instanovo (black), Instanovo v1.1 (blue)
...
Overall, Casanovo (violet) and Instanovo (black) inferred mostly identical peptides, with Casanovo inferred some peptides not seen in Instanovo, and a few exceptions where Instanovo peptides are slightly shorter/longer than their Casanovo counterparts. Instanovo v1.1 (blue) performed much better than the previous version, and slightly outperform Casanovo in some cases.
...
Similar patterns can be observed on the light chain, with the exception of Umab where only Instanovo v1.1 performed noticeably better.
...
for(i in 1:length(coverage_position_all)){
  coverage_position_count[[i]] <- tibble(seq_name = names(coverage_position_all[i]),
                                         ...
                                         Tool = rep(c("Casanovo", "Instanovo", "Instanovo_v1.1"), nchar(cdr3$amino_acid_sequence_Vregion[i])),
                                         count = 0)
}
...
```{r eval=FALSE, fig.height=2, fig.width=14, include=FALSE}
ggplot(data = coverage_position_count[[3]], aes(x = position, y = count, fill = factor(Tool, levels = c("Casanovo", "Instanovo")))) +
  ...
  geom_col() +
  scale_fill_manual(values = c("#aa50d0", "#030304", "#669bbc")) +
