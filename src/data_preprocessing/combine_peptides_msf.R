combine_peptides_msf <- function(input_folder) {
  ...
  combined <- lapply(peptide_files, function(pfile) {
    ...
    df <- df[, c("Peptide Sequence", "Protease", colnames(df)[grepl("_1 Intensity", colnames(df), fixed = TRUE)])]
    df <- reshape2::melt(df, id.vars = c("Peptide Sequence", "Protease"))
    colnames(df) <- c("Sequence", "Protease", "Rawfilenumber", "intensity")
    df$run <- strsplit(basename(pfile), "_")[[1]][1]
    df$Rawfilenumber <- stringr::str_remove(df$Rawfilenumber, "_1 Intensity")
    df[df$intensity > 0, ]
  }) |> dplyr::bind_rows()
  
  combined$tool <- "MSFragger"
